<script>
  ...
  import DynamicStageRenderer from './DynamicStageRenderer.svelte';
  ...
  export let useDynamicRenderer = true;
  
  // Determine if stage supports multiple events
...
{#if useDynamicRenderer}
  ...
  />
{:else}
<div class="stage-renderer cyber-panel p-6 transition-all duration-300 {isExpanded ? 'border-accent-primary' : ''}">
  <!-- Stage Header -->
  <div class="flex items-center gap-4">
    ...
    <div class="flex-1 min-w-0">
      ...
      <p class="text-sm text-text-secondary mb-2">
        {stage.description || `Stage: ${stage.id}`}
      </p>
          ...
          {#each stage.events as event, eventIndex}
            <div class="flex items-center gap-3 p-3 bg-bg-secondary/50 rounded-lg">
              ...
              <div class="flex-1 min-w-0">
                ...
                {#if event.details}
                  <p class="text-xs text-text-secondary">{event.details}</p>
                {/if}
              </div>
...
<style>
  ...
  .stage-renderer:last-child::before {
    display: none;
  }
