...
Has Technical Blueprint: ${!!questionnaireData.technical_blueprint}`;
  ...
  /**
   * Refine specification based on conversation history
   * Uses AI to consolidate Q&A into a structured spec
   * @param {Object} currentSpec - Current specification
   * @param {Array} conversationHistory - History of Q&A
   * @param {string} userMode - User mode
   * @returns {Promise<Object>} Refined specification
   */
  async refineSpec(currentSpec, conversationHistory, userMode) {
    try {
      structuredLogger.info('Refining specification based on conversation', {
        userMode,
        projectName: currentSpec.project_overview?.app_name,
        historyLength: conversationHistory.length
      });

      // If no history, return original spec
      ...
      const prompt = promptTemplateManager.getTemplate('refinement-consolidation', {
        specs_json: JSON.stringify(currentSpec, null, 2),
        conversation_history: JSON.stringify(conversationHistory, null, 2)
      });

      // Call AI model to consolidate specifications
      // Using 'refinement' task role which maps to appropriate model
      const response = await modelRouter.routeTask({
        role: 'system-architect', // Use a capable role for consolidation
        prompt,
        complexity: 'high'
      });

      let refinedSpecContent = response.content;

      // Extract JSON if wrapped in markdown blocks
      const jsonMatch = refinedSpecContent.match(/```json\s*([\s\S]*?)\s*```/) ||
        refinedSpecContent.match(/```\s*([\s\S]*?)\s*```/);

      if (jsonMatch) {
        refinedSpecContent = jsonMatch[1];
      }

      let refinedSpec;
      try {
        refinedSpec = JSON.parse(refinedSpecContent);
      } catch (parseError) {
        structuredLogger.error('Failed to parse refined spec JSON', {
          error: parseError.message,
          content: refinedSpecContent.substring(0, 200) + '...'
        });
        // Fallback to current spec if parsing fails, but attach history
        refinedSpec = { ...currentSpec };
      }

      // Ensure internal metadata is preserved/updated
      refinedSpec._clarification_history = conversationHistory;
      refinedSpec._last_refined_at = new Date().toISOString();

      return refinedSpec;
    } catch (error) {
      structuredLogger.error('Failed to refine specification', { error: error.message });
      throw error;
    }
  }
   ...
   */
  async generateDocs(spec) {
    try {
      structuredLogger.info('Generating documentation from specification', {
        projectName: spec.project_overview?.app_name
      });

      const promptTemplateManager = require('./prompt-templates');

      // Load prompt template
      // Note: In a real implementation, we would reuse the logic from PipelineOrchestrator
      // or move it to a shared service. For this implementation, we'll use a simplified version
      // or try to use the same template if available.

      let prompt;
      try {
        prompt = await promptTemplateManager.getTemplate('docs-creator', {
          specs_clean_json: JSON.stringify(spec, null, 2),
          clarification_history: JSON.stringify(spec._clarification_history || [], null, 2)
        });
      } catch (e) {
        // Fallback if template loading fails
        prompt = `Create comprehensive documentation for this app: ${JSON.stringify(spec)}`;
      }
      ...
      structuredLogger.error('Failed to generate documentation', { error: error.message });
      throw error;
    }
  }

  /**
   * Generate database and API schema from documentation
   * @param {string} docsMb - Documentation markdown
   * @returns {Promise<Object>} Schema JSON
   */
  async generateSchema(docsMd) {
    try {
      structuredLogger.info('Generating schema from documentation');
      ...
      const jsonMatch = schemaContent.match(/```json\s*([\s\S]*?)\s*```/) ||
        schemaContent.match(/```\s*([\s\S]*?)\s*```/);

      if (jsonMatch) {
        schemaContent = jsonMatch[1];
      }

      const schema = JSON.parse(schemaContent);
      return schema;
    } catch (error) {
      structuredLogger.error('Failed to generate schema', { error: error.message });
      throw error;
    }
  }

  /**
   * Generate file structure from documentation and schema
   * @param {string} docsMd - Documentation markdown
   * @param {Object} schema - Schema JSON
   * @returns {Promise<Object>} File structure JSON
   */
  async generateFileStructure(docsMd, schema) {
    try {
      ...
      let structureContent = response.content;

      // Extract JSON if wrapped
      const jsonMatch = structureContent.match(/```json\s*([\s\S]*?)\s*```/) ||
        structureContent.match(/```\s*([\s\S]*?)\s*```/);

      if (jsonMatch) {
        structureContent = jsonMatch[1];
      }

      const structure = JSON.parse(structureContent);
      return structure;
    } catch (error) {
      structuredLogger.error('Failed to generate file structure', { error: error.message });
      throw error;
    }
  }
}
