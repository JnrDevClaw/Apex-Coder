...
const { LLMProvider, LLMResponse } = require('./providers/llm-provider');
...
const AGENT_ROLES = {
  INTERVIEWER: {
    ...
    preferredModels: ['mistral', 'openrouter', 'gpt-4o-mini'],
    complexity: 'low',
...
class ModelRouter {
  ...
  async initialize(config = {}) {
    ...
    const routerConfig = {
      ...
      fallbackChain: config.fallbackChain ||
        (process.env.MODEL_ROUTER_FALLBACK_CHAIN || 'openrouter,deepseek,huggingface').split(','),
  ...
  loadProvidersFromEnv() {
    ...
    // ElectronHub (Primary provider - Zukijourney replacement)
    // Used for Stages 1.5, 3.5, 4, 5, 7 - GPT-4o, Claude 3.5
    if (process.env.ELECTRON_HUB_KEY) {
      providers.push({
        name: 'electronhub',
        type: 'ElectronHubProvider',
        config: {
          name: 'electronhub',
    ...
    // Gemini (Primary for main coding - Stage 7)
    // Using Gemini-3-flash-preview for code generation
